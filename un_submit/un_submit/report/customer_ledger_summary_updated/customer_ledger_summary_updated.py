...
class PartyLedgerSummaryReport:
    ...
    def prepare_invoiced_amounts(self):
        self.min_date = add_days(self.filters.to_date, -self.max_days) if self.max_days > 0 else self.filters.to_date
        self.invoiced_amounts = frappe._dict()
        for party in self.parties:
            self.invoiced_amounts[party] = {idx: 0.0 for idx in range(len(self.ranges))}
        invoice_dr_or_cr = "debit" if self.filters.party_type == "Customer" else "credit"
        voucher_types = ["Sales Invoice", "Journal Entry"] if self.filters.party_type == "Customer" else ["Purchase Invoice", "Journal Entry"]
        for gle in self.gl_entries:
            ...
            if (
                ...
                for idx, days_range in enumerate(self.ranges):
                    if days_ago <= days_range:
                        self.invoiced_amounts[gle.party][idx] += gle.get(invoice_dr_or_cr, 0)
    ...
    def get_average_outstanding_for_customer(self, customer, days_range):
        ...
        invoiced_amount = self.invoiced_amounts.get(customer, {}).get(idx, 0) if idx != -1 else 0
        avg_balance = max(closing_balance - invoiced_amount, 0)
        return avg_balance
