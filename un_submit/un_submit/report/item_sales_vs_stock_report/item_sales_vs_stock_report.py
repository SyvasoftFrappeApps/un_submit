...
def get_data(filters):
    ...
    return frappe.db.sql(f"""
        SELECT
            ...
            ROUND(
                (SUM(DISTINCT IFNULL(bin.actual_qty, 0)) / 
                NULLIF(SUM(IF(si.posting_date BETWEEN %(from_date)s AND %(to_date)s, si_item.qty, 0)), 0)),
                2
            ) AS custom_qty
        FROM
            tabItem item
        ...
        HAVING
            SUM(DISTINCT IFNULL(bin.actual_qty, 0)) > 0
        ORDER BY
            on_hand_qty DESC
